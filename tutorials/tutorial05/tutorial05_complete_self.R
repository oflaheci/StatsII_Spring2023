...
ord.logm <- polr(attitude ~ ., data = workingMoms, Hess = TRUE)
summary(ord.logm)
...
# run model
mult.logm <- multinom(attitude ~ ., data = workingMoms)
summary(mult.logm)
exp(coef(mult.logm))

# get p values
z <- summary(mult.logm)$coefficients/summary(mult.logm)$standard.errors
(p <- (1 - pnorm(abs(z), 0, 1)) * 2)

# how do we interpret these coefficients?

# we can use predicted probabilities to help our interpretation
ppm <- data.frame(fitted(mult.logm))
head(data.frame(attitude = workingMoms$attitude,
                SD = ppm$Strongly.Disagree,
                D = ppm$Disagree,
                A = ppm$Agree,
                SA = ppm$Strongly.Agree))
